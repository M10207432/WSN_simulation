...
static void EventSend(uint16, uint16);
int packetnum=1;
bool SendFlag=0;
/*********************************************************************
 * Packet Information
...
uint16 SimpleBLEPeripheral_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & SBP_PERIODIC_TASK1 )
  {
    /*
	//=========================Get event id
    uint16 events_flag=SBP_PERIODIC_TASK1>>4;
	uint8 events_id=0;
	while((events_flag && 0x0001)!=1){
		events_flag=events_flag>>1;
		events_id++;
	}
	
    //=========================Send packet
	PERIODIC_EVENT[events_id].EXESIZE=PERIODIC_EVENT[events_id].EXESIZE-6;
	if(PERIODIC_EVENT[events_id].EXESIZE<=0){
		PERIODIC_EVENT[events_id].EXESIZE=  PERIODIC_EVENT[events_id].SIZE;//Size reload 
		PERIODIC_EVENT[events_id].ARRIVAL=PERIODIC_EVENT[events_id].DEADLINE;
		PERIODIC_EVENT[events_id].DEADLINE=PERIODIC_EVENT[events_id].DEADLINE+PERIODIC_EVENT[events_id].PERIOD/1000;
		
		osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_TASK1, PERIODIC_EVENT[events_id].PERIOD );
	}else{
		osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_TASK1, 10);
	}
	...
	*/
        ...
        EventSend(id+1,PERIODIC_EVENT[id].ARRIVAL_COUNT );//osal_set_event( simpleBLEPeripheral_TaskID, SBP_BURST_EVT ); 
        EventSend(id+1,PERIODIC_EVENT[id].ARRIVAL_COUNT );//osal_set_event( simpleBLEPeripheral_TaskID, SBP_BURST_EVT ); 
        EventSend(id+1,PERIODIC_EVENT[id].ARRIVAL_COUNT );//osal_set_event( simpleBLEPeripheral_TaskID, SBP_BURST_EVT ); 
        EventSend(id+1, PERIODIC_EVENT[id].ARRIVAL_COUNT);//osal_set_event( simpleBLEPeripheral_TaskID, SBP_BURST_EVT ); 
        EventSend(id+1, PERIODIC_EVENT[id].ARRIVAL_COUNT);//osal_set_event( simpleBLEPeripheral_TaskID, SBP_BURST_EVT ); 
        ...
        /*
        //Check meet deadline
        if(PERIODIC_EVENT[id].READYFLAG==1){
          HalLcdWriteStringValue( "Miss Deadline", PERIODIC_EVENT[id].ARRIVAL_COUNT, 10,  HAL_LCD_LINE_4 );
        }else{
          
          PERIODIC_EVENT[id].EXESIZE=PERIODIC_EVENT[id].SIZE;//Reload
          PERIODIC_EVENT[id].READYFLAG=1;
          packetnum=packetnum+PERIODIC_EVENT[id].SIZE;
          ...
          HalLcdWriteStringValue( "Event0", PERIODIC_EVENT[id].ARRIVAL_COUNT, 10,  HAL_LCD_LINE_4 );
          PERIODIC_EVENT[id].ARRIVAL_COUNT=PERIODIC_EVENT[id].ARRIVAL_COUNT+1;
          PERIODIC_EVENT[id].DEADLINE=PERIODIC_EVENT[id].DEADLINE+PERIODIC_EVENT[id].PERIOD/1000;
        }
        */
 ...
 if ( events & SBP_BURST_EVT )
  {
    ...
    for(int c=0;c<1;c++){
      ...
      /*
		if(packetnum!=0){
			//Search the highest priority event
			...
			if((PERIODIC_EVENT[Queue[priority]].READYFLAG==1) && (PERIODIC_EVENT[Queue[priority]].EXESIZE>0)){
				...
				if(PERIODIC_EVENT[Queue[priority]].EXESIZE==0){
					...
					for(int i=0;i<AMOUNT_OF_EVENT;i++){
						if(PERIODIC_EVENT[i].READYFLAG!=0)
							stopflag=1;
					}
					if(stopflag==0)
...
static void EventSend(uint16 id,uint16 count){
	...
	Packet.len=20;
	Packet.handle=id;
	osal_memcpy( &Packet.value, &burstData,20);
	
	//Send
	if (GATT_Notification( gapConnHandle, &Packet, FALSE )==SUCCESS){}
