...
uint16 SimpleBLEPeripheral_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  /*==========================================
			SBP_PERIODIC_EVT(Periodic task)
  ==========================================*/
 ...
 if ( events & SBP_BURST_EVT )
  {
    ...
    if(testmode>=1){
      if(testmode==1){
          sendData();sendData();sendData();sendData();sendData();sendData();//6 packets
      }
      if(testmode==2){
...
static void simpleBLEPeripheral_HandleKeys( uint8 shift, uint8 keys )
{
  ...
  if(keys & HAL_KEY_CENTER){
      ...
      if(center_mode==0){
		center_mode=1;
		#if (defined HAL_LCD) && (HAL_LCD == TRUE)
			HalLcdWriteString( "Master",  HAL_LCD_LINE_4 );
		#endif
		...
		center_mode=0;
		#if (defined HAL_LCD) && (HAL_LCD == TRUE)
			HalLcdWriteString( "Slave",  HAL_LCD_LINE_4 );
		#endif // (defined HAL_LCD) && (HAL_LCD == TRUE)
...
static void sendData(void )
{
    ...
    if(testmode==0){
      if(burstData[0]==0x0A){
        osal_stop_timerEx(simpleBLEPeripheral_TaskID,SBP_BURST_EVT);
        HalLcdWriteString( "20KBytes:",  HAL_LCD_LINE_8 );
        counter = 0;
    ...
    /*
    if(burstData[0] == 5) //    if(counter >500) burstData[0] == 2
    {
		if(recevdata==1){
			osal_stop_timerEx(simpleBLEPeripheral_TaskID,SBP_BURST_EVT);
			counter = 0;
			...
			//--------------------------------------------------------------------------
			recevdata=11;
			HalLcdWriteStringValue( "Again:", (uint16)(recevdata), 10,  HAL_LCD_LINE_4 );
			SBP_BURST_EVT_PERIOD=16;
			packetnum=1;
			osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_BURST_EVT, SBP_BURST_EVT_PERIOD );
		}else{
...
static void simpleProfileChangeCB( uint8 paramID )
{
  ...
  switch( paramID )
  {
    case SIMPLEPROFILE_CHAR1:
	  ...
	  /*-----------------------------------------------
			Connection interval
			±µ¦¬8¥H¤Wªº­È·|³]©winterval¡A¤£·|°µµo°e
			±µ¦¬6¥H¤Uªº­È·|³]©wpacket number¡A·|°µµo°e
	  -----------------------------------------------*/
	   if(newValue>=8){
